...
// The few mdc components work as foundation/utility are not directly included webpack or
// material-component-web module. But still they are included since other component
// depends on them.
...
function checkJSDependencyAddedInWebpackConfig() {
  const jsconfig = WEBPACK_CONFIG.find((value) => {return value.name === 'js-components'});
...
function checkCSSDependencyAddedInWebpackConfig() {
  ...
  if (CSS_WHITELIST.indexOf(name) === -1) {
    const cssconfig = WEBPACK_CONFIG.find((value) => {return value.name === 'css'});
...
function checkCSSDependencyAddedInMDCPackage() {
  ...
  if (CSS_WHITELIST.indexOf(name) === -1) {
    ...
    assert.notEqual(typeof cssRules.find(value => {return value.href === cssRule}), 'undefined',
  ...
  traverse(ast, {
    'ExpressionStatement'({node}) {
      ...
      if (callee.object.name === 'autoInit' && callee.property.name === 'register') {
        const expression = args.find(value => {return value.type === 'MemberExpression'});
...
function checkComponentExportedAddedInMDCPackage(ast) {
  ...
  traverse(ast, {
    'ExportNamedDeclaration'({node}) {
      if (node.specifiers) {
        if (node.specifiers.find(value => {return value.exported.name === nameCamel})) {
          isExported = true;
        }
      }
    },
